...
library(Seurat) # 单细胞分析 # single cell analysis
library(GO.db) # GO注释 # GO annotation
library(biomaRt) # 基因ID转换 # gene ID conversion
library(RColorBrewer) # 调色板 # color palette

Sys.setenv(LANGUAGE = "en") #显示英文报错信息  # error messages are displayed in English
options(stringsAsFactors = FALSE) #禁止chr转成factor # chr is not allowed to be converted to factor
```

## 标准的seurat流程 Standard Seurat process
...
## 用不同颜色展示分组 Show the grouping with different colors

### 根据Patient_ID分组 Group according to Patient_ID
...
### 根据Diagnosis列分组 - 画图2D Grouping by Diagnosis Column - Paint 2D
...
### 根据细胞周期分组 - 画图2E Grouping according to cell cycle - Plot 2E
...
## 用渐变色展示pathway的表达量 Single-Cell Signature Explorer score is displayed with gradient color
...
### 使用Single-Cell Signature Explorer进行打分 Use the Single-Cell Signature Explorer for scoring
...
### 读取打分结果并画图 Read the scoring results and draw a graph
