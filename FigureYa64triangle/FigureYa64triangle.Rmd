...
library(ggtern) # 三角图 Triangle plot

# 如果需要使用本地源码版本（用于调试或特定版本需求），可以取消下面的注释
# If you need to use local source code version (for debugging or specific version requirements), uncomment the following
...
head(df)
# 使用安装的 ggtern 包时，size 应该作为 geom_point() 的参数，而不是在 aes() 中
# When using installed ggtern package, size should be a parameter of geom_point(), not in aes()
