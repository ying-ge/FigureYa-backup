...
library(tidyverse) # 数据处理和转换 Data manipulation and transformation
library(scales) # 数据缩放 Data scaling
library(ggplot2) # 数据可视化 Data visualization
library(directlabels) # 直接标签 Direct labels

# 加载 ggtern 本地源码（不依赖安装的包，兼容所有 ggplot2 版本）
# Load ggtern source code locally (without depending on installed package, compatible with all ggplot2 versions)
source("load_ggtern_local.R") # 从 GitHub 源码中提取的核心函数 Core functions extracted from GitHub source code

Sys.setenv(LANGUAGE = "en") #显示英文报错信息 display English error messages
options(stringsAsFactors = FALSE) #禁止chr转成factor prohibit the conversion of chr to factor
```

## 输入数据预处理 Preprocessing of input data
...
## 直接用原文数据画图 Draw a graph directly using the original data

```{r}
df <- read.csv("not_easy_input.csv")
head(df)
# 将 size 添加到数据中，并使用 scale_size 来确保 size 被正确处理
# Add size to data and use scale_size to ensure size is handled correctly
df$size <- df$Alteration.Frequency.in.glioma
ggtern(data=df, aes(x=Frequency.in.pGBM,
                    y=Frequency.in.LGG,
                    z=Frequency.in.sGBM,
                    size=size)) +  
   geom_point() +
   scale_size_continuous(range = c(1, 10))  # 确保 size 被正确处理 Ensure size is handled correctly
```
...
## 把size扩大50倍 Increase the size by 50 times
...
### 方案一：geom_text + AI Plan 1: geom_text + AI
...
### 方案二：用directlabels包 Plan 2: Use the directlabels package
