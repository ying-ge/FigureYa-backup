...
find_global_tern <- function (name, env=environment()){  
  ...
  if(!inherits(env,"environment")){stop("'env' must inherit the environment class")}
  if (exists(name, envir = env, inherits = FALSE)){return(get(name, envir = env, inherits = FALSE))}
  # 尝试从 ggtern 命名空间（如果已安装）
  # Try from ggtern namespace (if installed)
  if (requireNamespace("ggtern", quietly = TRUE)) {
    tryCatch({
      nsenv <- asNamespace("ggtern")
      if(exists(name, envir = nsenv)){return(get(name, envir = nsenv))}
    }, error = function(e) {})
  }
  # 从 ggplot2 命名空间查找
  # Find from ggplot2 namespace
  tryCatch({
    nsenv <- asNamespace("ggplot2")
    if(exists(name, envir = nsenv)){return(get(name, envir = nsenv))}
  }, error = function(e) {})
  # 如果都找不到，尝试一些已知的默认值
  # If all not found, try some known default values
  if (name == ".pt") {
    return(0.352777778)  # ggplot2 的默认点大小转换因子 Default point size conversion factor in ggplot2
  }
  if (name == ".base_to_ggplot") {
    # 从 colour 到 color 的映射 Mapping from colour to color
    return(c("colour" = "color"))
  }
  NULL
}
...
.zeroGrob      <- zeroGrob()  # 使用 ggplot2 的 zeroGrob 函数 Use ggplot2's zeroGrob function
...
tern_dep <- function(version, msg) {
  ...
  # 如果 ggtern 包未安装，跳过版本检查
  # If ggtern package is not installed, skip version check
  cv <- tryCatch({
    if (requireNamespace("ggtern", quietly = TRUE)) {
      packageVersion("ggtern")
    } else {
      # 使用默认版本号 Default version number
      as.package_version("3.4.2")
    }
  }, error = function(e) {
    as.package_version("3.4.2")
  })
