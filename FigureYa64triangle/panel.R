...
train_ranges <- function(panel, coord) {
  ...
  compute_range <- function(ix, iy) {
    ...
    # coord_train 是 coord 对象的方法，直接调用
    # coord_train is a method of coord object, call directly
    # 注意：scales 是一个列表，包含 x, y, T, L, R, W 等 scale 对象
    # Note: scales is a list containing x, y, T, L, R, W scale objects
