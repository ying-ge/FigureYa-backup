...
coord_transform.ternary <- function(coord, data, details, 
  ...
  if(!dont_transform){
    ...
    tryCatch({
      ...
      # 保存所有非坐标列，确保 size 等列被保留
      # Save all non-coordinate columns to ensure size and other columns are preserved
      non_coord_cols <- setdiff(names(data), c(ix.tern, ix.cart))
      if(length(non_coord_cols) > 0) {
        saved_cols <- data[, non_coord_cols, drop = FALSE]
      } else {
        saved_cols <- NULL
      }
      ...
      # 恢复所有非坐标列
      # Restore all non-coordinate columns
      if(!is.null(saved_cols) && nrow(saved_cols) == nrow(data)) {
        data <- cbind(data, saved_cols)
      }
      ...
      if(discard){
        ...
        # 使用索引来保留所有列，包括 size 等非坐标列
        # Use index to preserve all columns, including size and other non-coordinate columns
        keep_idx <- which(in.poly > 0)
        if(length(keep_idx) > 0) {
          data <- data[keep_idx, , drop = FALSE]
        } else {
          # 如果没有点保留，返回空数据框但保留列名
          # If no points kept, return empty data frame but preserve column names
  ...
  if(passToCartesian & !missing(details)){
    # 在 ggplot2 3.4.4 中，coord_transform.cartesian 可能不存在
    # In ggplot2 3.4.4, coord_transform.cartesian may not exist
    # 直接返回数据，保留所有列（包括 size）
    # Return data directly, preserving all columns (including size)
    # 注意：cartesian 转换通常不会改变列，所以直接返回数据
    # Note: cartesian transformation usually doesn't change columns, so return data directly
    data
...
coord_train.ternary <- function(coord, scales){
  ...
  # 使用 get_scales 方法获取 scale 对象
  # Use get_scales method to get scale objects
  x_scale <- scales$get_scales("x")
  y_scale <- scales$get_scales("y")
  
  if(is.null(x_scale) || is.null(y_scale)) {
    # 如果无法获取 scale，返回基本范围
    # If cannot get scales, return basic ranges
    ret <- list(x.range = coord$limits$x - hshift, y.range = coord$limits$y - vshift)
  } else {
    ret <- c(ggint$train_cartesian(x_scale,coord$limits$x - hshift, "x"),
             ggint$train_cartesian(y_scale,coord$limits$y - vshift, "y"))[c("x.range","y.range")]
  }
  ...
  for(ix in IX){
    # 使用 get_scales 方法获取 scale 对象
    # Use get_scales method to get scale object
    scale <- scales$get_scales(ix)
    if(!is.null(scale)){
  ...
  #Add the W label (arrow label suffix)
  # W scale 可能不存在，使用 tryCatch 获取或使用默认值
  # W scale may not exist, use tryCatch to get or use default value
  ret$Wlabel <- tryCatch({
    w_scale <- scales$get_scales("W")
    if(!is.null(w_scale)) w_scale$name else ""
  }, error = function(e) {
    ""  # 默认值为空字符串 Default value is empty string
  })
...
.rename_data_ternary <- function(coord,data){
  ...
  tryCatch({
    ...
    if(length(which(!frm %in% names(data))) == 0){
      names(to) <- frm
      data <- plyr::rename(data,to)#,warn_missing=FALSE)
    }
  },error=function(e){
    return(bup)
  })
