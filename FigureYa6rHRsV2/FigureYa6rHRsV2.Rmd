...
临床的亚组分析。例如，能从参与临床试验的那么多人当中发现EGFR抑制剂唯独对亚洲非吸烟女性有效。
这里只从形式上复现。FigureYa66UnivariateCox计算了单因素 HR，其输出文件cox_output.csv可以直接对接FigureYa6rHRsV2，只需调整列名。
。

Clinical subgroup analysis. For example, it was discovered that among the numerous participants in a clinical trial, EGFR inhibitors were effective only for Asian non-smoking females.
It is only reproduced here formally, if you need the calculation process, please refer to FigureYa66UnivariateCox.
The output file cox_output.csv can be used as input file for FigureYa6rHRsV2, only need to adjust the column names.
