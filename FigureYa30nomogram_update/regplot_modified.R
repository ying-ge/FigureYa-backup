...
if(FIRSTRUN) {
  ## Only close device if in clickable interactive mode
  ## For non-interactive PDF generation, preserve the existing device
  if(clickable && dev.cur() > 1) dev.off()
  ## Don't call plot.new() in non-interactive mode - it will clear the PDF
  ## if(!clickable && dev.cur() == 1) plot.new()
...
else  ##if(FIRSTRUN)
{
  ...
  {
  if(clickable && !is.null(bareplot)) {
    replayPlot(bareplot)
  } else {
    ## For non-interactive mode, just create a clean plot without replaying
    ## Don't reset FIRSTRUN to avoid redrawing everything
    plot(c(L-0.3*(M-L),M+0.1*(M-L)), c(0.5,npanels+2 + make_space),col="white" )
    ## FIRSTRUN <<- TRUE  ## REMOVED: This causes issues in non-interactive mode
  }
  }
} 
...
if(esc){
   ...
   ## In non-interactive mode, ensure the plot is fully rendered
   ## before returning
   if(!clickable) {
     ## Force rendering to the graphics device
     dev.control("enable")
     ## Ensure all pending graphics operations are complete
     dev.flush()
   }
