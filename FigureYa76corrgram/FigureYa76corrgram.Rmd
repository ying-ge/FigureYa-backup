...
同时展示每两个变量之间的相关关系和原始数据分布：

- 下三角：原始数据散点图 + 拟合线
- 上三角：相关系数热图
- 对角线：变量名称

每两个变量之间的相关关系，还可以画成圆形，可参考FigureYa37correlationV2_update和FigureYa81immune_network

Simultaneously display both the pairwise correlation relationships and the raw data distribution for every two variables:

- Lower triangle: Scatter plots of raw data with fitted (regression) lines
- Upper triangle: Correlation coefficient heatmap
- Diagonal: Variable names

For each pair of variables, the correlation relationship can also be visualized in a circular form. See examples in FigureYa37correlationV2_update and FigureYa81immune_network.

相关系数参考资料：<https://uc-r.github.io/correlations>，里面提到两个展示相关性的好用的画图工具：corrplot和corrgram。

- corrplot的用法可参考FigureYa70mutationEvents；
- corrgram基于pairs，包装进了几个现成的函数，这里借鉴了其中的panel.fill函数
...
- The usage of corrplot can refer to FigureYa70mutationEvents; 
- Corrgram is based on pairs and includes several ready-made functions. Here, we draw inspiration from the panel.fill function. 
...
关键是写好画小图的函数，然后用pairs组合成最终的效果。

The keypoint is to write a function for drawing small images and then combine them into the final effect using pairs.
