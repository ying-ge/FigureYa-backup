...
From<https://www.sciencedirect.com/science/article/pii/S2352396420300992>
...
直观展示两种方法的差异模式和一致性程度，通过散点分布和置信区间判断一致性范围，斜率检验的P值帮助判断是否存在系统性偏差。

适合连续性变量，需要评估可替换性，想发现系统性偏差的情况；

不适合分类变量（阳性/阴性），只关心总体准确率，样本量太小（<30个）。

AI时代涌现大量新方法、新工具，需要跟传统方法比较。

这不是简单的准确率问题，需要发现AI的系统性偏差：例如AI在轻度病例上表现很好，但在重度病例上系统性地低估严重程度。这种模式在Bland-Altman图上一目了然。

如果AI和经典方法的系统性偏差超过临床可接受范围，即使AI准确率很高，也不能直接替换使用。
...
It can be used to compare the agreement between different evaluations and to conduct consistency (slope) tests.

It is suitable for continuous variables, requires assessment of replaceability, and wants to find situations with systematic bias;

It is not suitable for categorical variables (positive/negative), only cares about overall accuracy, and the sample size is too small (<30).

There is a surge of new methods and tools in the AI era that need to be compared with traditional methods.

It is not a simple accuracy issue, but a need to find the systematic bias of AI: for example, AI performs well in mild cases, but systematically underestimates the severity in severe cases. This pattern is obvious in the Bland-Altman graph.

If the systematic bias between AI and classical methods exceeds the clinically acceptable range, AI cannot be directly replaced even if its accuracy is very high.
...
library(BlandAltmanLeh) # Bland-Altman相关函数 # Bland-Altman related functions
library(MethComp) # Bland-Altman相关函数 # Bland-Altman related functions
library(smatr) # 斜率检验相关函数 # Slope test related functions
...
## 分析并画图 Analyze and Draw Diagrams
