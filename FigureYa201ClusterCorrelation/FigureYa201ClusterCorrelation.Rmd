...
library(ConsensusClusterPlus) # 加载共识聚类包 # Load consensus clustering package
library(pheatmap) # 画热图 # heatmap package
...
easy_input_expr.txt, Contains the expression matrix of target genes.This file will be used to extract the expression profiles of the specified genes.

easy_input_cli.txt, Stores sample metadata for heatmap annotation (e.g., color-coding by clinical features).Format: Each row represents a sample; columns represent attributes (e.g., Gender, Status, Grade, Age).Note: Optional—omit this file if sample annotation is not required in the visualization.tores sample metadata for heatmap annotation (e.g., color-coding by clinical features).Format: Each row represents a sample; columns represent attributes (e.g., Gender, Status, Grade, Age).Note: Optional—omit this file if sample annotation is not required in the visualization.

easy_input_gene.txt, Lists target genes for clustering and plotting.Derived from the union of differentially expressed genes (DEGs) across three subtypes in Table S2 of the original publication.
...
- You may want the cluster corresponding to each sample for further survival analysis, differential expression analysis, etc.Open the ConsensusCluster folder, ConsensusCluster.k=X.consensusClass.csv corresponds to the cluster where the sample is located for each classification number.
- ConsensusCluster.k=X.consensusMatrix.csv corresponds to the heatmap in the consensus.pdf file, and you can use this matrix to DIY a better-looking heatmap.

## 计算基因与cluster的相关性 Calculating Gene-Cluster Correlations
