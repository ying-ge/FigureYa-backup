...
library(TCGAbiolinks) # TCGA数据下载工具 TCGA data download tool
library(sva) # 批次效应校正工具 Batch effect correction tool
library(cluster) # 聚类分析工具 Clustering analysis tool
library(oompaBase) # 绘图工具 Plotting tool
library(SummarizedExperiment) # 数据结构工具 Data structure tool
library(edgeR) # 差异表达分析工具 Differential expression analysis tool
source("batchPCA.R") # 加载绘制PCA图的自定义函数 Load custom function for PCA plotting
...
```{r}
## TCGA数据下载及预处理 TCGA data download and preprocessing
# 查询TCGA头颈鳞癌RNA-seq数据(STAR比对原始计数) Query TCGA HNSC RNA-seq data (STAR-aligned raw counts)
...
## 检查两类数据的量级 Checking data magnitude
...
## 合并数据集、检查批次效应 Merging datasets and checking batch effects
...
## ComBat去除批次效应 Batch effect correction using ComBat
```{r}
# 使用ComBat去除批次效应
