...
From <https://www.thno.org/v12p3104.htm>
...
library(SeuratData) # 单细胞数据集 SeuratData
library(ggplot2) # 绘图 Plotting
library(ggpubr) # 配色 Plotting
...
```{r}
# 设置抖动参数，防止数据聚集在一起 Setting jitter parameter to prevent data clustering
getScatterplot(pbmc3k, gene1 = "CD3G", gene2 = "CD247", 
               jitter.num = 0.15, pos = TRUE) + 
  theme(aspect.ratio = 1)
ggsave("correlation_dotplot.pdf")

# 不添加抖动，显示原始数据位置 No jitter, display original data position
getScatterplot(pbmc3k, gene1 = "CD3G", gene2 = "CD247", 
