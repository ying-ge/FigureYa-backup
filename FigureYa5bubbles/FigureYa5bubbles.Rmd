...
library(data.table) #数据处理 data processing
...
第二种：motif的pvalue，用homer找motif，产生knownResults.txt，其中包含了在peak（目标基因组区域）中富集的已知motif。一共10个sample，分别保存在10个文件夹里。

RNA-seq表达值，在终端里运行下面两行代码，下载并解压缩FPKM：

The first: Organize your data into easy_input.txt format, you can skip “Formatting input data” and go directly to “Start drawing”. The first column is the gene name, followed by the p-value of the motif in each sample, then the name of the motif, followed by the FPKM value.

The second: The p-value of motifs, using homer to find motifs, produces knownResults.txt, which contains known motifs enriched in the peak (target genomic region). There are 10 samples in total, which are saved in 10 folders respectively.
...
## 输入数据的格式化 Formatting input data
