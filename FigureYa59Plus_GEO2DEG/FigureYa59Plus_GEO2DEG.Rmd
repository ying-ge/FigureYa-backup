...
library(GEOquery) # 用于下载GEO数据库中的数据 # For downloading data from the GEO database
library(limma) # 用于差异表达分析 # For differential expression analysis
library(ggplot2) # 用于画图 # For plotting
library(ggrepel) # 用于避免标签重叠 # For avoiding label overlap
library(ggthemes) # 用于画图主题 # For plotting themes

Sys.setenv(LANGUAGE = "en") #显示英文报错信息 display English error messages
options(stringsAsFactors = FALSE) #禁止chr转成factor prohibit the conversion of chr to factor
```

## 输入数据的获得 Obtain input data
...
### 从GEO下载表达矩阵和芯片注释文件 Download expression matrix and chip annotation file from GEO
...
### 整理芯片平台注释信息 Organize chip platform annotation information
...
### 把表达矩阵的探针组ID换成Gene Symbol Replace the probe set ID of the expression matrix with Gene Symbol
...
### 按照实验设计整理分组信息 Organize group information according to experimental design
...
## 输入数据预处理 Input data preprocessing
...
### 离群sample的处理 Process outlier samples
