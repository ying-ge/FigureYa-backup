...
## 需求描述 Discription of Need

想实现10折交叉验证的分析方法，FigureYa182RFSurv和FigureYa198SignatureComb 都是根据基因的重要性筛选的，直接根据随机森林的准确度筛选，又是另一种不同的展示方式。

I want to implement a 10-fold cross-validation analysis method. Both FigureYa182RFSurv and FigureYa198SignatureComb select genes based on their importance, while selecting based on the accuracy of random forests offers a different perspective.

![](example.png)

From <https://www.aging-us.com/article/102814>
...
cox model with 10-fold cross-validation can be referred to FigureYa220repeatedLasso

## 环境设置 Setup Environment

```{r}
source("install_dependencies.R")
library(caret) # 机器学习 caret machine learning
library(survival) # 生存分析 survival analysis
library(shape) # 画箭头 draw arrows
...
## 单变量cox模型筛选基因 use univariate cox model to screen genes
...
## 随机森林筛选过程 Random Forest Feature Selection Process
...
```{r}
set.seed(19991018) # 设置种子 set seed for reproducibility
control <- rfeControl(functions = rfFuncs, # 选择随机森林 # random forest http://topepo.github.io/caret/recursive-feature-elimination.html#rfe
                      method = "LGOCV", # 选择交叉验证法 # choose cross-validation
                      number = 10) # 10折交叉验证 # 10-fold cross-validation
...
results <- rfe(x = tmp[,candidate.gene], 
               ...
               sizes = 1:(length(candidate.gene)-1), # 步长为1，速度较慢请耐心（约2小时） # step size is 1, it takes time (about 2 hours)
               rfeControl = control)
final.gene <- predictors(results) # 取出最终基因 # get final selected genes
write.table(final.gene,"output_selected features.txt",sep = "\t",row.names = F,col.names = F,quote = F)

accres <- results$results # 取出迭代结果 # get iteration results
...
index <- which.max(accres$Accuracy) # 取出准确率最大时的索引（基因个数) # get index of maximum accuracy (number of genes)
## 画圈圈
## Draw plot
