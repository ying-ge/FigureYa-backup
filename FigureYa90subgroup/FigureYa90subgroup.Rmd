...
library(tidyverse) # 数据处理和可视化 Data manipulation and visualization
library(survival) # 生存分析 Survival analysis
library(forestplot) # 森林图绘制 Forest plot drawing
...
## 计算HR和P value Calculate HR and P values
...
# 将HR列重命名为mean列，以便后续使用
# Rename HR column to mean for later use
colnames(forestplot_input)[colnames(forestplot_input) == "HR"] <- "mean"
# 注意：不再手动取对数，而是使用forestplot的xlog=TRUE参数自动处理
# Note: No manual log transformation, use xlog=TRUE parameter in forestplot instead
...
# 构建森林图的文本标签矩阵（使用原始HR值）
# Construct the text label matrix for the forest plot (using original HR values)
tabletext <- cbind(
...
# 使用默认参数绘制森林图（使用xlog=TRUE自动处理对数尺度）
# Draw a forest plot with default parameters (use xlog=TRUE for automatic log scale)
# 自动计算合理的x轴刻度
# Automatically calculate reasonable x-axis ticks
all_values <- c(data$CI_low_0.95, data$CI_up_0.95, 1)
all_values <- all_values[!is.na(all_values) & all_values > 0]
min_val <- min(all_values)
max_val <- max(all_values)

# 设置对数刻度：包含0.5, 1, 2或更宽的范围
# Set logarithmic ticks: include 0.5, 1, 2 or wider range
if (min_val < 0.5 && max_val > 2) {
...
forestplot(
  labeltext = tabletext,
  xlog = TRUE,  # 横坐标对数化（自动处理）
               # Logarithmic x-axis (automatic handling)
  mean = c(NA, NA, 1, data$mean, NA),  # HR值，参考线在1
               # HR values, reference line at 1
  lower = c(NA, NA, 1, data$CI_low_0.95, NA),  # 95%置信区间下限
               # Lower bound of 95% CI
  upper = c(NA, NA, 1, data$CI_up_0.95, NA),  # 95%置信区间上限
               # Upper bound of 95% CI
  xticks = xticks  # 自动计算的刻度
...
forestplot(
  ...
  xlog = TRUE,                    # 横坐标对数化（自动处理）
                                 # Logarithmic x-axis (automatic handling)
  mean = c(NA, NA, 1, data$mean, NA),  # HR值（中间值），参考线在1
                                 # HR values (median), reference line at 1
  lower = c(NA, NA, 1, data$CI_low_0.95, NA),  # 95%置信区间下限
  ...
  zero = 1,                # 参考线（无效线）的横坐标为1
                                 # X-coordinate of reference line at 1
  lwd.zero = 2,            # 参考线宽度
                                 # Reference line width
  grid = structure(c(data[1,]$mean), gp = gpar(col = "black", lty = 2, lwd = 2)),  # 网格线设置
                                 # Grid line settings
  xticks = xticks,         # 自动计算的X轴刻度
                                 # Automatically calculated X-axis ticks
  lwd.xaxis = 2,           # X轴线宽
                                 # X-axis line width
...
- **横坐标尺度**：森林图横坐标使用对数尺度（通过 `xlog=TRUE` 参数实现）
  **X-axis scale**: Forest plot x-axis uses logarithmic scale (via `xlog=TRUE` parameter)

- **参考线位置**：参考线位于 HR=1（无效线）
  **Reference line**: Reference line at HR=1 (null line)

- **数据处理**：直接使用原始 HR 值和置信区间，由 forestplot 包自动处理对数转换
  **Data processing**: Use original HR values and confidence intervals directly, forestplot package handles log transformation automatically

- **X轴标签**：显示为 "Favors Control" 和 "Favors Experimental" 方向
  **X-axis label**: Shows direction as "Favors Control" and "Favors Experimental"

- **X轴刻度**：根据数据范围自动计算合理的对数刻度（如 0.5, 1, 2, 5），确保所有置信区间都能清晰显示
  **X-axis ticks**: Automatically calculated logarithmic ticks based on data range (e.g., 0.5, 1, 2, 5) to ensure all confidence intervals are clearly displayed
