...
forestplot_input$mean <- log((forestplot_input$CI_low_0.95 + forestplot_input$CI_up_0.95)/2)  # 计算置信区间中点的对数值
                                      # Calculate log of midpoint of confidence interval
forestplot_input$CI_low_0.95 <- log(forestplot_input$CI_low_0.95)  # 置信区间下限取对数
                                      # Log transform lower CI bound
forestplot_input$CI_up_0.95 <- log(forestplot_input$CI_up_0.95)  # 置信区间上限取对数
                                      # Log transform upper CI bound
...
# 在绘制森林图时，表格中显示的HR值需要转换回原始尺度
# When plotting forest plot, HR values in the table need to be converted back to original scale
hr_original <- exp(data$mean)
ci_low_original <- exp(data$CI_low_0.95)
ci_up_original <- exp(data$CI_up_0.95)
...
tabletext <- cbind(
  ...
  c("Hazard Ratio\n(95% CI)", NA, NA, ifelse(!is.na(data$count), paste(round(hr_original, 2), " (", round(ci_low_original, 2), " to ", round(ci_up_original, 2),")", sep=""), NA), NA),
  c("P-value", NA, NA, ifelse(data$P < 0.001, "<0.001", round(data$P,3)), NA)
)
tabletext

# 使用默认参数绘制森林图（对数尺度）
# Draw a forest plot with default parameters (log scale)
forestplot(
  labeltext = tabletext,
  mean = c(NA, NA, 0, data$mean, NA),  # log(HR)值，参考线在0（对应HR=1）
                                         # log(HR) values, reference line at 0 (corresponding to HR=1)
  lower = c(NA, NA, 0, data$CI_low_0.95, NA),  # 95%置信区间下限的对数值
                                         # Log of lower bound of 95% CI
  upper = c(NA, NA, 0, data$CI_up_0.95, NA)  # 95%置信区间上限的对数值
                                         # Log of upper bound of 95% CI
)
...
forestplot(
  ...
  mean = c(NA, NA, 0, data$mean, NA),  # log(HR)值（中间值），参考线在0（对应HR=1）
                                 # log(HR) values (median), reference line at 0 (corresponding to HR=1)
  lower = c(NA, NA, 0, data$CI_low_0.95, NA),  # 95%置信区间下限的对数值
                                 # Log of lower bound of 95% confidence interval
  upper = c(NA, NA, 0, data$CI_up_0.95, NA),  # 95%置信区间上限的对数值
                                 # Log of upper bound of 95% confidence interval
  # title = "Hazard Ratio",
  ...
  zero = 0,                # 参考线（无效线）的横坐标为0（对应HR=1）
                                 # X-coordinate of reference line at 0 (corresponding to HR=1)
  lwd.zero = 2,            # 参考线宽度
                                 # Reference line width
  grid = structure(c(data[1,]$mean), gp = gpar(col = "black", lty = 2, lwd = 2)),  # 网格线设置
                                 # Grid line settings
  # xticks = c(round(min(data$mean),1), 0, round((max(data$mean)-min(data$mean)/2),1), round(max(data$mean),1)),  # 自定义X轴刻度
  lwd.xaxis = 2,           # X轴线宽
                                 # X-axis line width
  xlab = "log(Hazard Ratio)",   # X轴标题（对数尺度）
                                 # X-axis title (log scale)
...
- **横坐标尺度**：森林图横坐标从线性尺度改为对数尺度
  **X-axis scale**: Forest plot x-axis changed from linear to logarithmic scale

- **参考线位置**：参考线从 HR=1 移至 log(HR)=0
  **Reference line**: Reference line moved from HR=1 to log(HR)=0

- **数据显示**：表格显示原始 HR 值，图形使用对数尺度
  **Data display**: Table shows original HR values, plot uses logarithmic scale
