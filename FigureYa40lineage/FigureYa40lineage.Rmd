...
# 添加错误处理和超时机制 | Add error handling and timeout mechanism
tryCatch({
  # 设置超时机制 - 使用更小的数据集子集进行快速分析 | Set timeout mechanism - use smaller data subset for quick analysis
  if (nrow(CDS) > 1000) {
    cat("Dataset too large, using subset for demonstration...\n")
    # 随机选择1000个细胞进行演示 | Randomly select 1000 cells for demonstration
    set.seed(123)
    cell_subset <- sample(colnames(CDS), min(1000, ncol(CDS)))
    CDS <- CDS[, cell_subset]
    cat("Using", ncol(CDS), "cells for analysis\n")
  }

  cat("Starting DDRTree dimensionality reduction...\n")
  CDS <- reduceDimension(CDS, max_components = 2,
                          method="DDRTree")
  cat("DDRTree reduction completed successfully!\n")

}, error = function(e) {
  cat("DDRTree reduction failed:", e$message, "\n")
  cat("Using alternative PCA method...\n")
  # 备用PCA方法 | Alternative PCA method
  CDS <- reduceDimension(CDS, max_components = 2,
                          method="PCA")
  cat("PCA reduction completed as fallback!\n")
}, interrupt = function(e) {
  cat("Analysis interrupted, using PCA method...\n")
  CDS <- reduceDimension(CDS, max_components = 2,
                          method="PCA")
})
