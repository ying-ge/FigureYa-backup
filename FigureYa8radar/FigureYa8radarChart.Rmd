...
### 准备svg语句 Prepare the svg statement
#### 1.画4个圆 Draw 4 circles
...
#### 2.通过路径（path）画花瓣 Draw flower petals through a path

原理图 Schematic diagram

![](schematics.png)

绘图者 Drawn by Zhaodong Hao
...
#### 3.文本语句 Text statements

svg文本格式为\<text x y font-size fill >words\</text\>，x，y是文本左下角的坐标 The svg text is formatted as \<text x y font-size fill >words\</text\>, x, y are the coordinates of the lower left corner of the text
...
### 将准备好的svg语句写入.svg文件 Write prepared svg statements to .svg file

首先写入XML声明、svg dtd、svg代码开始标签\</svg> First write the XML declaration, svg dtd, svg code start tag \</svg>
...
### 写入准备好的画圆、花瓣、文本语句 Write the prepared drawing circles, petals, and text
