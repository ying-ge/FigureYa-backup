...
install_kpmt <- function() {
  if (!is_package_installed("kpmt")) {
    ...
    tryCatch({
      ...
      # Alternative: try to install from source if available
      tryCatch({
...
# First install kpmt (critical dependency for ChAMP)
install_kpmt()
...
cat("\nInstalling other Bioconductor packages...\n")
bioc_packages <- c("ChAMPdata", "clusterProfiler", "ComplexHeatmap", "GSVA", "impute",
...
critical_packages <- c("ChAMP", "ChAMPdata", "clusterProfiler", "ComplexHeatmap", "GSVA")
missing_packages <- c()
...
if (length(missing_packages) == 0) {
  ...
  for (pkg in missing_packages) {
    if (pkg == "ChAMP") {
      cat("  - ChAMP: BiocManager::install('ChAMP')\n")
      cat("  - Or: remotes::install_github('YuanTian1991/ChAMP')\n")
    } else {
...
test_packages <- c("ChAMP", "ChAMPdata", "ggplot2", "data.table")
