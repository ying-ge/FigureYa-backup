...
From <https://linkinghub.elsevier.com/retrieve/pii/S1535610822004998>
...
ifnb.rds，A Seurat object。如果你还没有这个文件，可以使用下面的代码从SeuratData包加载示例数据并保存为rds文件。

First run CellPhoneDB, then use its output files as input for plotting

ifnb.rds is a Seurat object. If you don't have this file yet, you can use the code below to load the example data from SeuratData package and save it as an rds file.
...
# Load ifnb dataset
LoadData("ifnb")
seu <- UpdateSeuratObject(ifnb)
saveRDS(seu, "ifnb.rds")  # 保存为rds文件以便下次使用
...
# 读取ifnb.rds文件（Seurat对象）
# Read ifnb.rds file (Seurat object)
