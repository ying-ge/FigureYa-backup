...
library(DealGPL570) 
library(stringr) # 用于字符串处理（for string processing）
library(sva) # 用于批次效应校正（for batch effect correction）
library(tibble) # 用于数据框操作（for data frame manipulation）
library(dplyr) # 用于数据操作（for data manipulation）
library(tidyverse) # 用于数据处理和可视化（for data processing and visualization）
library(GenomicFeatures) # 用于基因组特征处理（for genomic feature processing）
library(limma) # 用于差异表达分析（for differential expression analysis）
library(survival) # 用于生存分析（for survival analysis）
library(rtracklayer) # 用于基因组数据导入导出（for genomic data import/export）
...
### 来自GEO的表达数据处理 Expression data processing from GEO
...
## 对每个免疫细胞系的lncRNA进行排序 Sort the lncRNAs of each immune cell line
...
## 计算TSI，提取ihk-lnc Calculate TSI and extract ihk lnc
...
# GEO+CCLE整合数据的主成分分析（Principal component analysis for GEO+CCLE integrated data）
batchPCA(indata = t(scale(t(geo_ccle_combat))),
         batch = rep(c("Immune cell lines", "STOMACH cancer cell lines"),
                    times = c(114, 37)),
         fig.dir = ".",
         PCA.fig.title = "Combat PCA for GEO+CCLE Integration",
         showID = F,
         cex = 1.2,
         showLegend = T,
         pos = "topright")
...
## 构建TILSig打分 Build TILSig scoring system
