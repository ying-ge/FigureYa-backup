...
From <https://jitc.bmj.com/content/10/2/e003534>
...
library(Seurat) # 单细胞测序数据分析包 single-cell RNA-seq data analysis package
library(SeuratData) # 单细胞测序数据集 single-cell RNA-seq data set
library(ggplot2) # 绘图包 plotting package
library(RColorBrewer) # 颜色包 color package
library(dplyr) # 数据处理包 data processing package
library(magrittr) # 数据处理包 data processing package
library(monocle) # 发育轨迹分析包 developmental trajectory analysis package
...
## 输入文件及预处理 Input Files and Preprocessing
...
## 构建monocle对象 Constructing the monocle object
...
## 确定用于轨迹排序的基因 Determining Genes for Trajectory Ordering
...
# 修复 monocle 与新版 igraph 的兼容性问题
# Fix compatibility issue between monocle and new igraph version
# 临时替换 igraph::dfs 函数以支持新版 API（graph.dfs 内部调用 dfs）
# Temporarily replace igraph::dfs function to support new API (graph.dfs calls dfs internally)
if (packageVersion("igraph") >= "1.3.0") {
  # 保存原始的 dfs 函数
  # Save original dfs function
  original_dfs <- igraph::dfs
  
  # 创建兼容的 dfs 函数，将 neimode 参数转换为 mode
  # Create compatible dfs function, convert neimode parameter to mode
  compatible_dfs <- function(graph, root, ..., neimode = NULL) {
    # 如果提供了 neimode 参数，转换为 mode
    # If neimode is provided, convert to mode
    if (!is.null(neimode)) {
      # 调用新版本的 dfs，使用 mode 参数替代 neimode
      # Call new version dfs with mode parameter instead of neimode
      return(original_dfs(graph = graph, root = root, mode = neimode, ...))
    } else {
      # 如果没有提供 neimode，使用默认值
      # If neimode not provided, use default
      return(original_dfs(graph = graph, root = root, ...))
    }
  }
  
  # 临时替换 igraph 命名空间中的 dfs 函数
  # Temporarily replace dfs function in igraph namespace
  tryCatch({
    # 获取 igraph 命名空间
    # Get igraph namespace
    igraph_ns <- as.environment("package:igraph")
    
    # 解锁并替换 dfs 函数
    # Unlock and replace dfs function
    if (bindingIsLocked("dfs", igraph_ns)) {
      unlockBinding("dfs", igraph_ns)
    }
    assign("dfs", compatible_dfs, igraph_ns)
    lockBinding("dfs", igraph_ns)
    
    cat("Applied compatibility patch for igraph dfs function\n")
  }, error = function(e) {
    cat("Warning: Could not patch igraph dfs function. Error:", e$message, "\n")
    cat("Trying alternative method...\n")
    # 尝试使用 assignInNamespace
    # Try using assignInNamespace
    tryCatch({
      assignInNamespace("dfs", compatible_dfs, ns = "igraph")
      cat("Applied compatibility patch using assignInNamespace\n")
    }, error = function(e2) {
      cat("Warning: Alternative method also failed. Error:", e2$message, "\n")
      cat("The code may fail with newer igraph versions.\n")
    })
  })
}
...
## 确定热图中Cell fate 1和2对应的轨迹 Determining the Trajectories Corresponding to Cell Fate 1 and 2 in the Heatmap
